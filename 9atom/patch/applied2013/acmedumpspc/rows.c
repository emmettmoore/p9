...
rowadd(Row *row, Column *c, int x)
{
	...
	c->row = row;
	c->tag.row = row;
	row->col = erealloc(row->col, (row->ncol+1)*sizeof(Column*));
...
rowgrow(Column *c, int but)
{
  ...
  Found:
	r1 = row->r;
	r1.min.y = row->col[0]->r.min.y;
	r = c->r;
	if(i == 0 && row->ncol > 1 && r.max.x == r1.max.x){	/* unhide */
		draw(screen, c->r, display->white, nil, ZP);
		r.max.x = r1.max.x-(row->ncol-1)*(80+Scrollwid+Border);
		colresize(c, r);
		i++;
		for(; i < row->ncol; i++){
			r.min.x = r.max.x;
			...
			r.min.x = r.max.x;
			r.max.x += 80+Scrollwid;
			d = row->col[i];
			colresize(d, r);
		}
		return;
	}
	if(but == 3){	/* leave alone */
		if(i != 0){	/* swap with first */
			d = row->col[0];
			row->col[0] = c;
			row->col[i] = d;
			i = 0;
	...
	if(but == 2){	/* max size */
		do{
			r = c->r;
			rowgrow(c, 0);
		}while(r.max.x-r.min.x < c->r.max.x-c->r.min.x);
		return;
	}
	if(but == -1){	/* grow to right */
		if(r1.max.x-(row->ncol-1-i)*(80+Scrollwid+Border) <= c->r.max.x)
			return;
		d = row->col[i+1];
		r = d->r;
		w = r.max.x-r.min.x;
		r.min.x += 80+Scrollwid;
		if(r.max.x-r.min.x < 80+Scrollwid)
			r.min.x = r.max.x-(80+Scrollwid);
		if(w == r.max.x-r.min.x){
			if(i < row->ncol-2 && r1.max.x-i*(80+Scrollwid+Border) > c->r.max.x){
				rowgrow(d, -1);
				rowgrow(c, -1);
			}
			return;
	...
	if(but == -2){	/* grow to left */
		if(r1.min.x+i*(80+Scrollwid+Border) >= c->r.min.x)
			return;
		d = row->col[i-1];
		r = d->r;
		w = r.max.x-r.min.x;
		r.max.x -= 80+Scrollwid;
		if(r.max.x-r.min.x < 80+Scrollwid)
			r.max.x = r.min.x+80+Scrollwid;
		if(w == r.max.x-r.min.x){
			if(i > 1 && r1.min.x+i*(80+Scrollwid+Border) < c->r.min.x){
				rowgrow(d, -2);
				rowgrow(c, -2);
			}
			return;
	...
	/* grow */
	if(i == 0 ||
	(r1.min.x+i*(80+Scrollwid+Border) >= c->r.min.x &&
	r1.max.x-(row->ncol-i-1)*(80+Scrollwid+Border) > c->r.max.x) ||
	(i < row->ncol-1 &&
	row->col[i+1]->r.max.x-row->col[i+1]->r.min.x >
	row->col[i-1]->r.max.x-row->col[i-1]->r.min.x))
		rowgrow(c, -1);
	else
		rowgrow(c, -2);
...
void
rowdragcol(Row *row, Column *c, int but)
{
	Rectangle r;
	int i, b, x;
	Point p, op;
...
rowclose(Row *row, Column *c, int dofree)
{
  ...
  Found:
	...
	row->ncol--;
	row->col = erealloc(row->col, row->ncol*sizeof(Column*));
	if(row->ncol == 0){
		draw(screen, r, display->white, nil, ZP);
		return;
	}
...
rmcmds(Rune *r, int n)
{
	...
	for(c = command; c != nil; c = c->next){
		cmd = runesmprint("%S ", c->name);
		cmd[c->nname] = 0;
		p = runestrstr(r, cmd);
		if(p){
			cnt = c->nname;
			cnt += isspacerune(p[c->nname]);
			memmove(p, p+cnt, (n+1 - (p-r + cnt))*sizeof(Rune));
			n -= cnt;
		}
		free(cmd);
	}
...
rowdump(Row *row, char *file)
{
	...
	m = min(RBUFSIZE, row->tag.file->nc);
	bufread(row->tag.file, 0, r, m);
	n = 0;
	while(n<m && r[n]!='\n')
		n++;
	n = rmcmds(r, n);
	...
	for(i=0; i<row->ncol; i++){
		c = row->col[i];
		m = min(RBUFSIZE, c->tag.file->nc);
		bufread(c->tag.file, 0, r, m);
		n = 0;
		while(n<m && r[n]!='\n')
			n++;
		Bprint(b, "c%11d %.*S\n", i, n, r);
...
rowload(Row *row, char *file, int initing)
{
		...
		case 'c':
			...
			for(n=0; n<nr; n++)
				if(r[n] == ' ')
					break;
			if(n != nr && i < row->ncol){
				textdelete(&row->col[i]->tag, 0, row->col[i]->tag.file->nc, TRUE);
		...
		case 'w':
			...
			for(n=0; n<nr; n++)
				if(r[n] == ' ')
					break;
			if(n != nr){
				textdelete(&row->tag, 0, row->tag.file->nc, TRUE);
	...
	for(; l != nil; l = rdline(b, &line)){
		...
		if(ndumped >= 0){
			...
			snprint(buf, BUFSIZE, "/tmp/d%d.%.4sacme", getpid(), getuser());
