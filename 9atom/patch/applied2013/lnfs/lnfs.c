...
usage(void)
{
	fprint(2, "usage: %s [-l len] [-r] [-s srvname] mountpoint\n", argv0);
	exits("usage");
}
 ...
 *  the short name is the first namelen bytes of the base64
 *  encoding of the MD5 hash of the longname.
 */
#include <avl.h>

typedef	struct	Name	Name;
typedef	struct	Aname	Aname;
...
anamefree(void *v)
{
	...
	a = v;
	if(a->name->ref-- == 0)
		free(a->name);			/* assume 1 allocation */
	free(a);
}

int
namecmp(Avl *va, Avl *vb)
{
	int r, type;
	Name *a, *b;
...
static void
insert(Avltree *t, Name *n, int type)
{
	Avl *old;
	Aname *a;
...
static Name*
lookup(Avltree *t, char *s, int type)
{
	Name n;
	Aname a, *p;
	...
	a.name = &n;
	a.type = type;
	if(p = (Aname*)lookupavl(t, &a))
		return p->name;
	return nil;
}

static void
delete(Avltree *t, char *s, int type)
{
	Name n;
	Aname a, *p;
...
static void
freeavl(Avl *a, void (freefn)(void*))
{
	if(a == nil)
		return;
	freeavl(a->n[0], freefn);
	freeavl(a->n[1], freefn);
	freefn(a);
}

static void
freeavltree(Avltree *t, void (freefn)(void*))
{
	if(t == nil)
		return;
	freeavl(t->root, freefn);
	free(t);
}

Dir	*dbstat;				/* last stat of the name file */
char	*namefile = "./.longnames";
static	Avltree	*shorttolong;
static	Avltree	*longtoshort;

Name*
newname(char *lname, int writeflag)
{
	int l, fd;
	Name *n;
	...
	l = strlen(lname);
	md5((uchar*)lname, l, digest, nil);
	n = emalloc(sizeof *n + (l + 1) + namelen);
	n->s[Long] = ((char*)n) + sizeof *n;
	n->s[Short] = ((char*)n) + sizeof *n + (l + 1);
	memcpy(n->s[Long], lname, (l + 1));
	enc32(n->s[Short], namelen, digest, MD5dlen);

	insert(shorttolong, n, Short);
	insert(longtoshort, n, Long);
...
newnames(void)
{
	freeavltree(longtoshort, anamefree);
	freeavltree(shorttolong, anamefree);

	shorttolong = mkavltree(namecmp);
	longtoshort = mkavltree(namecmp);
}

/*
 *  reread the file if the qid.path has changed.
 *  read any new entries if length has changed.
 *  bug: we force ourselves to reread this.  :-(
 */
void
readnames(void)
{
	...
	/* not multi-user safe.  alternative: lock files, etc. */
	if(once != 0)
		return;
	newnames();
	once = 1;
	...
	while((p = Brdline(b, '\n')) != nil){
		p[Blinelen(b)-1] = 0;
		if(lookup(longtoshort, p, Long) == nil)
			newname(p, 0);
	}
	Bterm(b);
	dbstat = d;
 ...
 */
char*
long2short(char *lname, int writeflag)
{
	Name *n;

	if(strlen(lname) < namelen && (frogs || strpbrk(lname, " ")==nil))
		return lname;
	if((n = lookup(longtoshort, lname, Long)) != nil)
		return n->s[Short];

	if(!writeflag)
		return lname;
	return newname(lname, !readonly)->s[Short];
}
 ...
 */
char*
short2long(char *sname)
{
	Name *n;

	if((n = lookup(shorttolong, sname, Short)) != nil)
		return n->s[Long];
	return sname;
}
