...
cpuidentify(void)
{
	...
	memmove(m->cpuidid,   &regs.bx, BY2WD);	/* bx */
	memmove(m->cpuidid+4, &regs.dx, BY2WD);	/* dx */
	memmove(m->cpuidid+8, &regs.cx, BY2WD);	/* cx */
	m->cpuidid[12] = '\0';
	 ...
	 */
	if((m->cpuiddx & Pae) == 0)
		panic("cpu doesn't support pae");

	if(m->cpuiddx & (Pge|Pae|Mce|Pse)){
		cr4 = 0;
		if(m->cpuiddx & Pae)
			cr4 |= getcr4() & 1<<5;
		if(m->cpuiddx & Pse)
...
archctlread(Chan*, void *a, long nn, vlong offset)
{
	...
	if(coherence == mb586)
		...
		p = seprint(p, ep, "%#p\n", coherence);
	p = seprint(p, ep, "cmpswap cmpswap486\n");
	p = seprint(p, ep, "i8253set %s\n", doi8253set ? "on" : "off");
	p = seprint(p, ep, "fp sse\n");
	USED(p);
...
static long
msrwr(Chan *c, void *va, long n, vlong)
{
	char buf[32];
	u32int msr;
	...
	memmove(buf, va, n);
	buf[n] = 0;
	msr = strtoul(buf, nil, 0);
	c->aux = (void*)(uintptr)msr;

	return n;
}

static long
msrrd(Chan *c, void *va, long n, vlong offset)
{
	char *a, buf[64];
	long i, t, r;
	u32int msr;
	...
	a = va;
	t = 0;
	msr = (u32int)(uintptr)c->aux;
	for(i = 0; i < conf.nmach; i++){
		procwired(up, i);
		sched();
		rdmsr(msr, &v);
		snprint(buf, sizeof buf, "%ld %.16llux\n", i, v);
		r = readstr(offset, a, n, buf);
		if(r == 0)
			break;
		offset -= r;
		if(offset < 0)
...
archinit(void)
{
	...
	addarchfile("msr", 0664, msrrd, msrwr);
}
