...
#define dprint(...)	{if(debug)iprint(__VA_ARGS__); }while(0)
static int debug;
...
rtl8169mii(Ctlr* ctlr)
{
	...
	if((mii = miiattach(ctlr, (1<<1), rtl8169miimir, rtl8169miimiw)) == nil)
		return nil;
	...
	if(miistatus(mii) < 0 || mii->curphy->speed < 1000){
		miireset(mii);
		miiane(mii, ~0, ~0, ~0);

		/* try to bring the link up here */
		for(i = 0; i < 350; i += 1){
...
static u32int
ethercrcbe(uchar *addr, usize len)
{
	int i, j;
	u32int c, crc, carry;
...
rtl8169multicast(void* ether, uchar *eaddr, int add)
{
	...
	if (ctlr->pcie) {
		csr32w(ctlr, Mar0,   swab32(ctlr->mchash>>32));
		csr32w(ctlr, Mar0+4, swab32(ctlr->mchash));
	} else {
...
static long
rtl8169ifstat(Ether* edev, void* a, long n, usize offset)
{
	char *p0, *p, *e;
	Ctlr *ctlr;
	Dtcc *dtcc;
...
static long
rtl8169ctl(Ether *e, void *buf, long n)
{
	Cmdbuf *cb;
	...
	if(cb->nf == 1 && strcmp(cb->f[0], "debug") == 0)
		debug ^= 1;
	if(cb->nf == 1 && strcmp(cb->f[0], "link") == 0)
		rtl8169link(e);
	else
		cmderror(cb, "unknown");
	free(cb);
