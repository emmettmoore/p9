...
static long
igbeifstat(Ether* edev, void* a, long n, usize offset)
{
	Ctlr *ctlr;
	char *p0, *p, *e, *s;
	int i, r;
...
igberbfree(Block* b)
{
	b->rp = b->wp = (uchar*)ROUNDUP((uintptr)b->base, BY2PG);
...
igbetransmit(Ether* edev)
{
	...
	for(;;){
		if((b = qget(edev->oq)) == nil)
			break;
		td = &c->tdba[tdt];
		td->addr[0] = Pciwaddrl(b->rp);
		td->addr[1] = Pciwaddrh(b->rp);
		td->control = Ifcs|Teop|DtypeDD|Rs|BLEN(b);
		c->tb[tdt] = b;
		c->sent++;
		tdt = Next(tdt, m);
		if(Next(tdt, m) == tdh){
...
{
	int i;
	u32int ctrl, r, pause, swdpio, txcw;
