...
typedef struct Controller {
	...
	uintmem scriptpa;			/* where the real script is */
	Pcidev* pcidev;
	SDev*	sdev;
...
sd53c8xxpnp(void)
{
	...
	while((p = pcimatch(p, NCR_VID, 0)) != nil && nctlr > 0){
		...
		if(regpa == 0)
			continue;
		ba = 2 + (regpa & 0x04) != 0;
		regva = vmap(regpa & ~(uintmem)0xf, p->mem[1].size);
		if(regva == nil)
			continue;
		...
		if((v->feature & LocalRAM) && sizeof(na_script) <= 4096){
			...
			scriptva = vmap(scriptpa & ~(uintmem)0xf, p->mem[ba].size);
			if(scriptva)
				script = scriptva;
		}
		if(scriptpa == 0){
