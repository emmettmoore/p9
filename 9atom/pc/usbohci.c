 ...
 *
 * BUGS:
 * - not really 64-bit safe (Tds aren't necessarly in low memory)
...
struct Ed {
	u32int	ctrl;
	u32int	tail;		/* transfer descriptor */
	u32int	head;
	u32int	nexted;
...
struct Isoio
{
	...
	u32int	frno;		/* next frame number avail for I/O */
	u32int	left;		/* remainder after rounding Hz to samples/ms */
...
{
	u32int	ctrl;
	u32int	cbp;		/* current buffer pointer */
	u32int	nexttd;
	u32int	be;
	u16int	offsets[8];	/* used by Iso Tds only */
...
struct Ohci
{
	...
	u32int	cmdsts;			/*08*/
	u32int	intrsts;			/*0c*/
	u32int	intrenable;		/*10*/
	u32int	intrdisable;		/*14*/

	/* memory pointer group */
	u32int	hcca;			/*18*/
	u32int	periodcurred;		/*1c*/
	u32int	ctlheaded;		/*20*/
	u32int	ctlcurred;		/*24*/
	u32int	bulkheaded;		/*28*/
	u32int	bulkcurred;		/*2c*/
	u32int	donehead;		/*30*/

	/* frame counter group */
	u32int	fminterval;		/*34*/
	u32int	fmremaining;		/*38*/
	u32int	fmnumber;		/*3c*/
	u32int	periodicstart;		/*40*/
	u32int	lsthreshold;		/*44*/

	/* root hub group */
	u32int	rhdesca;		/*48*/
	u32int	rhdescb;		/*4c*/
	u32int	rhsts;			/*50*/
	u32int	rhportsts[15];		/*54*/
	u32int	pad25[20];		/*90*/

	/* unknown */
	u32int	hostueaddr;		/*e0*/
	u32int	hostuests;		/*e4*/
	u32int	hosttimeoutctrl;		/*e8*/
	u32int	pad59;			/*ec*/
	u32int	pad60;			/*f0*/
	u32int	hostrevision;		/*f4*/
	u32int	pad62[2];
					/*100*/
};
...
static void*
lomallocalign(usize sz, usize align)
{
	void *va;
...
tdalloc(void)
{
	...
	if(tdpool.free == nil){
		...
		sz = ROUNDUP(sizeof *td, Align);
		mem = lomallocalign(Incr*sz, Align);
		if(mem == nil)
			panic("tdalloc");
		for(i = 0; i < Incr; i++){
			td = (Td*)(mem + i*sz);
...
seprinttd(char *s, char *e, Td *td, int iso)
{
	...
	s = seprint(s, e, "%#p ep %#p ctrl %#.8ux", td, td->ep, td->ctrl);
	s = seprint(s, e, " cc=%#ux", (td->ctrl >> Tdccshift) & Tdccmask);
	if(iso == 0){
		...
		if((td->ctrl & Tdusetog) != 0)
			...
			s = seprint(s, e, " d-");
		s = seprint(s, e, " ec=%ud", (td->ctrl >> Tderrshift) & Tderrmask);
	}else{
		s = seprint(s, e, " fc=%ud", (td->ctrl >> Tdfcshift) & Tdfcmask);
		s = seprint(s, e, " sf=%ud", td->ctrl & Tdsfmask);
	}
...
dumped(Ed *ed)
{
	...
	if(buf == nil)
		return;
	e = buf+512;
	s = seprint(buf, e, "\ted %#p: ctrl %#.8ux", ed, ed->ctrl);
	if((ed->ctrl & Edskip) != 0)
		s = seprint(s, e, " skip");
	if((ed->ctrl & Ediso) != 0)
		s = seprint(s, e, " iso");
	...
	if(edhalted(ed))
		s = seprint(s, e, " hlt");
	s = seprint(s, e, " ep%ud.%ud", (ed->ctrl>>7)&Epmax, ed->ctrl&0x7f);
	s = seprint(s, e, " maxpkt %ud", (ed->ctrl>>Edmpsshift)&Edmpsmask);
	seprint(s, e, " tail %#.8ux head %#.8ux next %#.8ux\n", ed->tail, ed->head, ed->nexted);
	print("%s", buf);
	free(buf);
...
interrupt(Ureg *, void *arg)
{
	...
	if((status & Ue) != 0){
		...
		print("ohci: unrecoverable error frame %#.8ux ed %#.8ux, "
			"ints %d %d %d %d\n",
	...
	if(status != 0)
		print("ohci interrupt: unhandled sts %#.8ux\n", status);
	iunlock(ctlr);
}
...
epgettd(Ep *ep, Qio *io, Td **dtdp, int flags, void *a, int count)
{
	...
	if(count <= BY2PG)
		...
		bp->rp = (uchar*)ROUNDUP((uintptr)bp->rp, BY2PG);
		bp->wp = bp->rp;
	}
	dtd = *dtdp;
	td = dtd;
...
portstatus(Hci *hp, int port)
{
	...
	if(v & (HPstatuschg|HPchange))
		ddprint("ohci port %d sts %#ux hub sts %#x\n", port, ohcistatus, v);
	return v;
}
...
dumpohci(Ctlr *ctlr)
{
	...
	for(i = 0; i < sizeof(Ohci)/sizeof *ohci; i++)
		if(i < 3 || ohci[i] != 0)
			print("\t[%#2.2x]\t%#8.8ux\n", i * 4, ohci[i]);
	print("\n");
}
...
scanpci(void)
{
	...
	for(p = nil; p = pcimatch(p, 0, 0); ) {
		...
		dprint("ohci: %.4ux/%.4ux port %#P size %#ux irq %d\n",
			p->vid, p->did, pa, p->mem[0].size, p->intl);
		if(i == Nhcis){
			print("ohci: %T ignored; increase Nhcis\n", p->tbdf);
			continue;
		}
		va = vmap(pa, p->mem[0].size);
		if(va == nil){
