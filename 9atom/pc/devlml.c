...
lmlreset(void)
{
	...
	for(nlml = 0; nlml < NLML && (pcidev = pcimatch(pcidev, VENDOR_ZORAN,
		...
		print("zr36067 found at %#P", pcidev->mem[0].bar & ~(uintmem)0xf);

		regpa = pcidev->mem[0].bar & ~(uintmem)0xf;
...
lmlread(Chan *c, void *va, long n, vlong voff)
{
	...
	case Qctl0:
		if(i >= nlml)
			error(Eio);
		lml = lmls+i;
		len = snprint(lmlinfo, sizeof lmlinfo,
			"lml%djpg	lml%draw\n"
			"lml%d.regs	%#P	%#ux\n"
			"lml%d.mjpg	%#P	%#ux\n",
			i, i,
			i, lml->pcidev->mem[0].bar & ~(uintmem)0xf, lml->pcidev->mem[0].size,
