...
uvlong intrtimetot[256];
uvlong intrcountx[256];

int (*traplapiceoi)(int);
...
intrdisable(int irq, void (*f)(Ureg *, void *), void *a, int tbdf, char *name)
{
	...
	for(vno = 0; vno < 256; vno++)
		for(pv = vctl + vno; v = *pv; pv = &v->next)
			if(v->irq == irq)
			if(v->f == f)
			if(v->tbdf == tbdf)
			if(v->a == a)
	...
	for(vno=0; vno<nelem(vctl); vno++){
		for(v=vctl[vno]; v; v=v->next){
			m = snprint(str, sizeof str, "%11d %11d %20llud %20llud %-*.*s %.*s\n",
				vno, v->irq, intrcountx[vno], intrtimetot[vno], 8, 8, v->type, KNAMELEN, v->name);
...
dumpregs(Ureg* ureg)
{
	 ...
	 */
	iprint("  CR0 %.8ux CR2 %.8ux CR3 %.8ux",
		getcr0(), getcr2(), getcr3());
	if(m->cpuiddx & (Mce|Tsc|Pse|Vmex)){
		iprint(" CR4 %.8ux", getcr4());
...
syscall(Ureg* ureg)
{
	...
	if(up->nerrlab){
		...
		for(i = 0; i < NERR; i++){
			...
			if(up->errlab[i].sp == 0 && up->errlab[i].pc == 0)
				break;
		}
		panic("error stack");
	}
...
notify(Ureg* ureg)
{
	...
	if(strncmp(n->msg, "sys:", 4) == 0){
		l = strlen(n->msg);
		snprint(n->msg+l, ERRMAX-l, " pc=%#p", ureg->pc);
	}
