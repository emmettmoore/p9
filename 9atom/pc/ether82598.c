...
static Ctlrtype cttab[Nctlrtype] = {
	...
	x552,	9*1024,		Fphyspd|Ftxctl,		"x552",
};
...
static int
replenish(Ether *e, Ctlr *c, uint rdh, int maysleep)
{
	int rdt, m, i;
	Block *b;
	Rd *r;
...
eeload(Ctlr *c)
{
	...
	for(i = 0; i < 0x40; i++)
		u +=  eeread(c, i);
	for(i = 3; i < 0xf; i++){
		if((c->type == x540 || c->type == x552) && (i == 4 || i == 5))
			continue;
		p = eeread(c, i);
		l = eeread(c, p++);
		if((int)p+l+1 > 0xffff)
...
didtype(int d)
{
	...
	case 0x1560:
		return x540;
	case 0x15aa:	/* x552 backplane */
	case 0x15ab:	/* x552 backplane */
	case 0x15ac:	/* x552 sfp+ */
	case 0x15ad:	/* x552/x557-at 10gbase-t */
		return x552;
	default:
		return -1;
	}
