...
axpalloc(int ctlrno, Pcidev* pcidev)
{
	...
	if((addr = vmap(bar & ~(uintmem)0xf, pcidev->mem[0].size)) == 0){
		print("%s: can't map registers at %#P\n", ctlr->name, bar);
		return axpdealloc(ctlr);
	}
	ctlr->reg = addr;
	print("%s: port %#P irq %d ", ctlr->name, bar, pcidev->intl);

	/*
	 * Local address space 0.
	 */
	bar = pcidev->mem[2].bar;
	if((addr = vmap(bar & ~(uintmem)0xf, pcidev->mem[2].size)) == 0){
		print("%s: can't map memory at %#P\n", ctlr->name, bar);
		return axpdealloc(ctlr);
	}
	ctlr->mem = addr;
	ctlr->gcb = (Gcb*)(ctlr->mem+0x10000);
	print("mem %#P size %d: ", bar, pcidev->mem[2].size);
