...
ifstat(Ether* ether, void* a, long n, ulong offset)
{
	...
	len += snprint(p+len, READSTR-len, "transmit underrun errors: %ud\n", dump[3]);
	len += snprint(p+len, READSTR-len, "transmit lost carrier sense: %ud\n", dump[4]);
	len += snprint(p+len, READSTR-len, "transmit deferred: %ud\n", dump[5]);
	len += snprint(p+len, READSTR-len, "transmit single collisions: %ud\n", dump[6]);
	len += snprint(p+len, READSTR-len, "transmit multiple collisions: %ud\n", dump[7]);
	len += snprint(p+len, READSTR-len, "transmit total collisions: %ud\n", dump[8]);
	len += snprint(p+len, READSTR-len, "receive good frames: %ud\n", dump[9]);
	len += snprint(p+len, READSTR-len, "receive CRC errors: %ud\n", dump[10]);
	len += snprint(p+len, READSTR-len, "receive alignment errors: %ud\n", dump[11]);
	len += snprint(p+len, READSTR-len, "receive resource errors: %ud\n", dump[12]);
	len += snprint(p+len, READSTR-len, "receive overrun errors: %ud\n", dump[13]);
	len += snprint(p+len, READSTR-len, "receive collision detect errors: %ud\n", dump[14]);
	len += snprint(p+len, READSTR-len, "receive short frame errors: %ud\n", dump[15]);
