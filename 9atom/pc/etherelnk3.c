...
typedef struct Pd {
	u32int	np;			/* next pointer */
	u32int	control;		/* FSH or UpPktStatus */
	u32int	addr;
	u32int	len;
...
typedef struct Ctlr {
	...
	int	upenabled;
	int	dnenabled;
	uintmem	cbfnpa;			/* CardBus functions */
	u32int*	cbfn;
} Ctlr;
...
static uchar*
startdma(Ether* ether, uintmem address)
{
	int port, status, w;
	uchar *wp;
...
interrupt(Ureg*, void* arg)
{
	...
	do{
		...
		if(status & interruptMask)
			panic("#l%d: interrupt mask %#ux", ether->ctlrno, status);
...
tcm59Xpci(void)
{
	...
	while(p = pcimatch(p, 0x10B7, 0)){
		...
		case 0x5157:
			...
			ctlr->cbfnpa = p->mem[2].bar & ~(uintmem)0xf;
...
etherelnk3reset(Ether* ether)
{
	...
	case xcvrMii:
		...
		if(ctlr->did == 0x5157)
			...
			phyaddr = 24;
		for(i = 0; i < 7; i++)
			XCVRDEBUG(" %.2ux", miir(port, phyaddr, i));
			XCVRDEBUG("\n");
		...
		for(timeo = 0; timeo < 30; timeo++){
			...
			if(phystat & 0x20)
				break;
			XCVRDEBUG(" %.2ux", phystat);
			delay(100);
		}
		XCVRDEBUG(" %.2ux", miir(port, phyaddr, 0x01));
		XCVRDEBUG("\n");
