...
mga2164wenable(VGAscr* scr)
{
	...
	if(p->did == MGA2064){
		scr->mmio = vmap(p->mem[0].bar&~(uintmem)0xf, p->mem[0].size);
		if(scr->mmio == nil)
			return;
		addvgaseg("mga2164wmmio", p->mem[0].bar&~(uintmem)0xf, p->mem[0].size);
		vgalinearaddr(scr, p->mem[1].bar&~(uintmem)0xf, 8*MB);
	}else{
		scr->mmio = vmap(p->mem[1].bar&~(uintmem)0xf, p->mem[1].size);
		if(scr->mmio == nil)
			return;
		addvgaseg("mga2164wmmio", p->mem[1].bar&~(uintmem)0xf, p->mem[1].size);
		vgalinearaddr(scr, p->mem[0].bar&~(uintmem)0xf, 16*MB);
	}
	if(scr->paddr)
