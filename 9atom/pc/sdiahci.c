...
static Alist*
ahcibuild(Aportm *m, int rw, void *data, uint ss, uint n, vlong lba)
{
	uchar *c;
	uint flags;
	Alist *l;
...
ahcibio(SDunit *u, int lun, int write, void *a, long count, uvlong lba)
{
	...
	for(try = 0; try < 10;){
		...
		if(n > max)
			n = max;
		qlock(&d->portm);
		ahcibuild(&d->portm, rw, data, d->secsize, n, lba);
