...
typedef struct Ctlr {
	...
	uint	dperr;			/* system errors */
	uint	rmabt;
	uint	rtabt;
	uint	sserr;
	uint	rxsover;

	u32int	version;		/* silicon version; register 0x58h */
} Ctlr;
...
#define csr16w(c, r, l)	(outs((c)->port+(r), (u16int)(l)))
...
ifstat(Ether* ether, void* a, long n, ulong offset)
{
	...
	if(p == nil)
		error(Enomem);
	l = snprint(p, READSTR, "Rxa: %ud\n", ctlr->rxa);
	l += snprint(p+l, READSTR-l, "Rxo: %ud\n", ctlr->rxo);
	l += snprint(p+l, READSTR-l, "Rlong: %ud\n", ctlr->rlong);
	l += snprint(p+l, READSTR-l, "Runt: %ud\n", ctlr->runt);
	l += snprint(p+l, READSTR-l, "Ise: %ud\n", ctlr->ise);
	l += snprint(p+l, READSTR-l, "Fae: %ud\n", ctlr->fae);
	l += snprint(p+l, READSTR-l, "Lbp: %ud\n", ctlr->lbp);
	l += snprint(p+l, READSTR-l, "Tfu: %ud\n", ctlr->tfu);
	l += snprint(p+l, READSTR-l, "Txa: %ud\n", ctlr->txa);
	l += snprint(p+l, READSTR-l, "CRC Error: %ud\n", ctlr->crce);
	l += snprint(p+l, READSTR-l, "Collision Seen: %ud\n", ctlr->col);
	l += snprint(p+l, READSTR-l, "Frame Too Long: %ud\n", ctlr->rlong);
	l += snprint(p+l, READSTR-l, "Runt Frame: %ud\n", ctlr->runt);
	l += snprint(p+l, READSTR-l, "Rx Underflow Error: %ud\n", ctlr->rxorn);
	l += snprint(p+l, READSTR-l, "Tx Underrun: %ud\n", ctlr->txurn);
	l += snprint(p+l, READSTR-l, "Excessive Collisions: %ud\n", ctlr->ec);
	l += snprint(p+l, READSTR-l, "Late Collision: %ud\n", ctlr->owc);
	l += snprint(p+l, READSTR-l, "Loss of Carrier: %ud\n", ctlr->crs);
	l += snprint(p+l, READSTR-l, "Parity: %ud\n", ctlr->dperr);
	l += snprint(p+l, READSTR-l, "Aborts: %ud\n", ctlr->rmabt+ctlr->rtabt);
	l += snprint(p+l, READSTR-l, "RX Status overrun: %ud\n", ctlr->rxsover);
...
sisrdcmos(Ctlr *ctlr)
{
	...
	debug("ns83815: SiS 630 rev. %ux reading mac addr from cmos via bridge at port %#ux\n", ctlr->pcidev->rid, port);
...
reset(Ether* ether)
{
	...
	for(ctlr = ctlrhead; ctlr != nil; ctlr = ctlr->next){
		if(ctlr->active)
			continue;
		if(ethercfgmatch(ether, ctlr->pcidev, ctlr->port) == 0){
			ctlr->active = 1;
			break;
		}
	}
