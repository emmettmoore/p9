...
static void
synccpus(volatile int *cntp, int n)
{
	ainc(cntp);
	while (*cntp < n)
		;
...
pass1(int pass, volatile Diag *dp)
{
	...
	ilock(dp);
	if(dp->cnt != 0)
		panic("cpu%d: diag: failed w count %d", m->machno, dp->cnt);
	iunlock(dp);
...
l1diag(void)
{
	...
	if(dp->sync < navailcpus || dp->sync >= 2 * navailcpus)
		panic("cpu%d: diag: failed w dp->sync %d", m->machno,
			dp->sync);
	if(dp->cnt != 0)
		panic("cpu%d: diag: failed w dp->cnt %d", m->machno,
			dp->cnt);
