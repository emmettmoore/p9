...
static int
setupcons(Uart *p, int on)
{
	int r;
	static Lock conslk;
	static Queue *lastcons;

	r = 0;
	lock(&conslk);
	if(on && !p->console && lastcons == nil){
		if(!p->enabled)
			uartenable(p);
		if(!p->enabled){
			/* qopen will fail early in boot (no malloc) */
			consuart = p;
			unlock(&conslk);
			return up == nil? 0: -1;
		}
...
uartrecv(Uart *p,  char ch)
{
	...
	if(p->putc)
		p->putc(p->iq, ch);
	else if (p->iw != nil) {		/* maybe the line isn't enabled yet */
		ilock(&p->rlock);
		next = p->iw + 1;
		if(next == p->ie)
			next = p->istage;
...
uartgetc(void)
{
	...
	if(consuart == nil || (getc = consuart->phys->getc) == nil)
		return -1;
	return getc(consuart);
}
...
uartputc(int c)
{
	...
	if(consuart == nil || (putc = consuart->phys->putc) == nil)
		return;
	putc(consuart, c);
}
