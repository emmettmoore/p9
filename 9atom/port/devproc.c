...
static int	procctlkmemio(Proc*, uintptr, int, void*, int);
...
static long 
·fpudevprocio(Proc *p, void *va, long n, uvlong offset, int write)
{
	uchar *fp;
	int sz;
...
long (*fpudevprocio)(Proc*, void*, long, uvlong, int) = ·fpudevprocio;
...
procread(Chan *c, void *va, long n, vlong off)
{
	...
	case Qsyscall:
		if(p->syscalltrace == nil)
			return 0;
		return readstr(offset, a, n, p->syscalltrace);

	case Qmem:
		if(offset < KZERO)
			return procctlmemio(p, offset, n, va, 1);
		else
			return procctlkmemio(p, offset, n, va, 1);
	...
	case Qfpregs:
		return fpudevprocio(p, va, n, offset, 0);
...
procwrite(Chan *c, void *va, long n, vlong off)
{
	...
	case Qmem:
		if(offset < KZERO){
			...
			n = procctlkmemio(p, offset, n, va, 0);
		break;
...
procctlmemio(Proc *p, uintptr offset, int n, void *va, int read)
{
	...
	if(pte == 0)
		panic("procctlmemio");
	pg = pte->pages[(soff&(s->ptemapmem-1))>>s->lgpgsize];
	if(pagedout(pg))
		panic("procctlmemio1");
...
static int
procctlkmemio(Proc*, uintptr offset, int n, void *va, int read)
{
	int i;
	void *kva;
	Confmem *cm;
	...
	if(!iseve())
		error(Eperm);
	/* write is never safe, but sometimes useful */
//	if(!read && (1 || getconf("*insecure") == nil))
//		error(Eperm);
	kva = nil;
	...
	for(i=0; kva==nil && i<nelem(conf.mem); i++){
		cm = &conf.mem[i];
