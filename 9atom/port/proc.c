...
schedinit(void)		/* never returns */
{
	...
	if(up) {
		...
		case Moribund:
			 ...
			 * 	procalloc
			 *	palloc.avail[PGSHIFT]	TO DO: eliminate
			 */
			mmurelease(up);

			up->qnext = procalloc.free;
			procalloc.free = up;
			unlock(&palloc.avail[PGSHIFT]);
			unlock(&procalloc);
			break;
		}
		up->mach = nil;
...
sched(void)
{
	...
	if(m->ilockdepth)
		panic("mach%d: ilockdepth %d, last lock %#p at %#p, sched called from %#p",
			...
			(up != nil && up->lastilock != nil)? lockgetpc(up->lastilock): (uintptr)0,
			getcallerpc(&p+2));
	if(up){
		/*
		...
		if(up->nlocks && up->state != Moribund){
			if(up->delaysched > 1000000)
				iprint("%s: delaysched: %#p %ld\n", up->text, lockgetpc(up->lastlock), up->delaysched);
			up->delaysched++;
 			delayedscheds++;
			return;
		}
...
newproc(void)
{
	...
	for(;;) {
		if(p = procalloc.free)
			break;
panic("no procs; %s forking (%ld avail)", up? up->text: "kernel", conf.nproc);
...
pexit(char *exitstr, int freemem)
{
	...
	if(up->kp == 0) {
		...
		if(p == 0) {
			if(exitstr == 0)
				exitstr = "unknown";
			panic("boot process died: %s: %s", up->text, exitstr);
		}
...
dumpaproc(Proc *p)
{
	...
	print("%3lud:%10s pc %#p dbgpc %#p %8s (%s) ut %ld st %ld bss %lux qpc %#p nl %ud nd %lud lpc %#p pri %lud\n",
		...
		p->time[0], p->time[1], bss, p->qpc, p->nlocks, p->delaysched, lockgetpc(p->lastlock), p->priority);
}
...
killbig(char *why)
{
	...
	print("%lud: %s: %s: killed: %s\n", kp->pid, kp->text, kp->user, why);
	for(p = procalloc.arena; p < ep; p++) {
		if(p->state == Dead || p->kp)
			continue;
