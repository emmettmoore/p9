...
#define LPTR(q)		((u32int*)KADDR((q) & ~0x1F))
...
struct Isoio
{
	...
	union{
		...
		u32int**	tdps;	/* same thing, as seen by hw */
	};
};
...
struct Itd
{
	u32int	link;		/* to next hw struct */
	u32int	csw[8];		/* sts/length/pg/off. updated by hw */
	u32int	buffer[7];	/* buffer pointers, addrs, maxsz */
	u32int	xbuffer[7];	/* high 32 bits of buffer for 64-bits */

	u32int	_pad0;		/* pad to next cache line */
	/* cache-line boundary here */

	/* software */
	Itd*	next;
	uint	ndata;		/* number of bytes in data */
	uint	mdata;		/* max number of bytes in data */
	uchar*	data;
};
...
{
	u32int	link;		/* to next hw struct */
	u32int	epc;		/* static endpoint state. addrs */
	u32int	mfs;		/* static endpoint state. µ-frame sched. */
	u32int	csw;		/* transfer state. updated by hw */
	u32int	buffer[2];	/* buf. ptr/offset. offset updated by hw */
				/* buf ptr/TP/Tcnt. TP/Tcnt updated by hw */
	u32int	blink;		/* back pointer */
	/* cache-line boundary after xbuffer[0] */
	u32int	xbuffer[2];	/* high 32 bits of buffer for 64-bits */
...
struct Td
{
	u32int	nlink;		/* to next Td */
	u32int	alink;		/* alternate link to next Td */
	u32int	csw;		/* cmd/sts. updated by hw */
	u32int	buffer[5];	/* buf ptrs. offset updated by hw */
	/* cache-line boundary here */
	u32int	xbuffer[5];	/* high 32 bits of buffer for 64-bits */
...
struct Qh
{
	u32int	link;		/* to next Qh in round robin */
	u32int	eps0;		/* static endpoint state. addrs */
	u32int	eps1;		/* static endpoint state. µ-frame sched. */

	/* updated by hw */
	u32int	tclink;		/* current Td (No Term bit here!) */
	u32int	nlink;		/* to next Td */
	u32int	alink;		/* alternate link to next Td */
	u32int	csw;		/* cmd/sts. updated by hw */
...
static char*
seprintlink(char *s, char *se, char *name, u32int l, int typed)
{
...
epgettd(Qio *io, int flags, void *a, int count, int maxpkt)
{
	...
	if(count <= Align - sizeof(Td)){
		...
		td->data = (uchar*)ROUNDUP(PTR2UINT(td->buff), 0x1000);
	}

	pa = PADDR(td->data);
	pae = pa + count;
	if(sizeof(uintptr)>4 && (uint)pae != pae)
		panic("usbehci: pa above 4g");
	for(i = 0; i < nelem(td->buffer); i++){
		td->buffer[i] = pa;
		pa &= ~0xFFF;
		pa += 0x1000;
