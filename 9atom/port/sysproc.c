...
sysrfork(ulong *arg)
{
	...
	p->fpsave = up->fpsave;			/* expensive save of fcr */
	...
	p->fpstate = FPinit;			/* regs are dead on function call */
	pid = p->pid;
...
sysexec(ulong *arg)
{
	...
	img = attachimage(SG_TEXT|SG_RONLY, tc, UTZERO, t);
	ts = img->s;
	up->seg[TSEG] = ts;
	ts->flushme = 1;
	ts->fstart = 0;
