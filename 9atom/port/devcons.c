...
void
newcons(void *v, int crnl, void (*puts)(void*, char*, int), int (*qlen)(void*))
{
	static Lock lk;

	ilock(&lk);
	if(nxkbd + 1 >= nelem(xkbd)){
		iunlock(&lk);
		return;
	}
	xkbd[nxkbd].v = v;
	xkbd[nxkbd].crnl = crnl;
	xkbd[nxkbd].puts = puts;
	xkbd[nxkbd].qlen = qlen;
	nxkbd++;
	iunlock(&lk);
}
...
xecho(char *buf, int n)
{
	...
	for(i = 0; i < nxkbd; i++){
		...
		while(n-- > 0){
			if(p >= e){
				xkbd[i].puts(xkbd[i].v, ebuf, p - ebuf);
				p = ebuf;
			}
			x = *buf++;
			if(x == '\n' && xkbd[i].crnl){
...
consread(Chan *c, void *buf, long n, vlong off)
{
	...
	case Qmach:
		return readnum((ulong)offset, buf, n, m->machno, NUMSIZE);
	...
	case Qswap:
		snprint(tmp, sizeof tmp,
			...
			pagesmade()-pagesfree(), conf.upages,
