...
printmesg(char *fmt, uchar *a, int plsprnt)
{
	...
	*q++ = *a++;
	for(p=fmt; *p; p++){
		switch(*p){
		case 'l':
			q = seprint(q, e, " %ld", (long)BGLONG(a));
			a += 4;
			break;
		case 'L':
			q = seprint(q, e, " %.8lux", (ulong)BGLONG(a));
			a += 4;
			break;
		case 'R':
			q = seprint(q, e, " [%d %d %d %d]", BGLONG(a), BGLONG(a+4), BGLONG(a+8), BGLONG(a+12));
			a += 16;
			break;
		case 'P':
			q = seprint(q, e, " [%d %d]", BGLONG(a), BGLONG(a+4));
			a += 8;
			break;
		case 'b':
			q = seprint(q, e, " %d", *a++);
			break;
		case 's':
			q = seprint(q, e, " %d", BGSHORT(a));
			a += 2;
			break;
		case 'S':
			q = seprint(q, e, " %.4ux", BGSHORT(a));
			a += 2;
			break;
		}
	}
