...
struct Ctlr
{
	...
	u32int*	frames;		/* frame list (used by hw) */
...
struct Qh
{
	u32int	link;		/* link to next horiz. item (eg. Qh) */
	u32int	elink;		/* link to element (eg. Td; updated by hw) */

	u32int	state;		/* Qidle -> Qinstall -> Qrun -> Qdone | Qclose */
...
struct Td
{
	u32int	link;		/* Link to next Td or Qh */
	u32int	csw;		/* control and status word (updated by hw) */
	u32int	token;		/* endpt, device, pid */
	u32int	buffer;		/* buffer pointer */

	Td*	next;		/* in qh or Isoio or free list */
	u32int	ndata;		/* bytes available/used at data */
...
{
	s = seprint(s, se, "%#p link %#ux", td, td->link);
	if((td->link & Tdvf) != 0)
		s = seprint(s, se, "V");
	if((td->link & Tdterm) != 0)
		s = seprint(s, se, "T");
	...
	if((td->csw & (Tderr1|Tderr2)) == 0)
		s = seprint(s, se, "z");
	if(td->csw & Tderrors)
		s = seprint(s, se, " err %#ux", td->csw & Tderrors);
	if(td->csw & Tdstalled)
		s = seprint(s, se, "s");
	if(td->csw & Tddberr)
		s = seprint(s, se, "d");
	...
	s = seprint(s, se, " token %#ux", td->token);
	...
	if(td->token & Tddata1)
		...
		s = seprint(s, se, " d0");
	s = seprint(s, se, " id %#ux:", (td->token>>15) & Epmax);
	s = seprint(s, se, "%#ux", (td->token>>8) & Devmax);
	switch(tdtok(td)){
	case Tdtokin:
		s = seprint(s, se, " in");
		break;
	...
	s = seprint(s, se, " ndata %ud sbuff %#p buff %#p",
...
epiowait(Ctlr *ctlr, Qio *io, int tmout, ulong load)
{
	...
	qh = io->qh;
	ddqprint("uhci io %#p sleep on qh %#p state %ud\n", io, qh, qh->state);
	timedout = 0;
	if(waserror()){
...
scanpci(void)
{
	...
	while(p = pcimatch(p, 0, 0)){
		 ...
		 */
		if(p->ccrb != 0xc || p->ccru != 3)
			continue;
		switch(p->ccrp){
		case 0:
			io = p->mem[4].bar & ~(uintmem)0xf;
			break;
		default:
			continue;
		}
...
init(Hci *hp)
{
	...
	dprint(" flb %#ux frno %#ux psc0 %#ux psc1 %#ux",
