...
TEXT mode32bit(SB), 1, $0
	...
	MOVL	$PADDR(CPU0PDB), DI		/* clear pages for the tables etc. */
	XORL	AX, AX
	MOVL	$(CPU0END-CPU0PDB), CX
	SHRL	$2, CX
	...
	MOVL	$PADDR(CPU0PTE), BX		/* PTE's for KZERO */
	ADDL	$(PTEWRITE|PTEVALID), BX	/* page permissions */
	MOVL	$((CPU0GDT-CPU0PTE)/BY2PG), CX	/* pages of page tables */
...
_setpde:
	...
	MOVL	$(PTEWRITE|PTEVALID), BX	/* phys address 0 and page permissions */
	MOVL	$(BOOTMEMSIZE/BY2PG), CX			/* pages in 8MB */
_setpte:
	MOVL	BX, (AX)
	ADDL	$(1<<PGSHIFT), BX
	ADDL	$BY2PTE, AX
 ...
 */
	MOVL	CR0, CX
	ANDL	$~0x80000000, CX	/* paging must be off for switch to PAE */
	MOVL	CX, CR0
	...
	MOVL	$PADDR(CPU0PDPT), BX		/* page directory pointer table */
	MOVL	$PADDR(CPU0PDB), CX		/* load address of page directory */
	MOVL	(PDO(KZERO))(CX), AX		/* double-map KZERO at 0 */
	MOVL	AX, (PDO(0))(CX)
	ORL	$PTEVALID, CX
	MOVL	CX, (PDPTO(KZERO))(BX)			/* page directory table for KZERO */
	MOVL	CX, (PDPTO(0))(BX)			/* double map KZERO at 0 */
	MOVL	BX, CR3
	DELAY					/* JMP .+2 */
...
TEXT _startpg(SB), 1, $0
	...
	MOVL	$0, (PDPTO(0))(BX)		/* undo double-map of KZERO at 0 */
	MOVL	$CPU0PDB, CX		/* undo double-map of KZERO in page directory */
	MOVL	$0, (PDO(0))(CX)
...
TEXT cycles(SB), 1, $0				/* time stamp counter */
	RDTSC
...
TEXT x87off(SB), 1, $0				/* disable */
	X87OFF(l1)
	RET

TEXT x87init(SB), 1, $0				/* enable and init */
	X87ON
	FINIT
	WAIT
...
TEXT ssesave(SB), 1, $0
	...
	ADDL	$15, AX
	ANDL	$~15, AX
	BYTE $0x0f; BYTE $0xae; BYTE $0x00;	/* FXSAVE	0(AX) */
	RET

TEXT sserestore(SB), 1, $0
	X87ON
	MOVL	p+0(FP), AX
	ADDL	$15, AX
	ANDL	$~15, AX
	BYTE $0x0f; BYTE $0xae; BYTE $0x08;	/* FXRSTOR	0(AX)	*/
	WAIT					/* gratitous x87 support?	*/
	RET
...
TEXT ldmxcsr(SB), 1, $0
	...
	PUSHL		AX
/*	LDMXCSR	0(SP) */
	BYTE $0x0f; BYTE $0xae; BYTE $0x14; BYTE $0x24;
	POPL		AX
	RET

TEXT sseclear(SB), 1, $0				/* clear pending exceptions */
	X87ON
	PUSHL		$(1<<11 | 1<<12)
/*	LDMXCSR	0(SP) */
	BYTE $0x0f; BYTE $0xae; BYTE $0x14; BYTE $0x24;
...
_mmstart32:
	...
	MOVL	BP, AX				/* linear address to monitor */
	XORL	CX, CX				/* extensions */
	XORL	DX, DX				/* hints */
	MONITOR
	...
	/*XORL	CX, CX*/			/* extensions (different from monitor) */
	XORL	AX, AX				/* hints */
	MWAIT
...
TEXT vectortable(SB), 1, $0
	...
	CALL _strayintr(SB); BYTE $0x13		/* SIMD exception */
