...
#define csr32w(c, r, l)	(outl((c)->port+((r)*8), (u32int)(l)))
...
ifstat(Ether* ether, void* a, long n, ulong offset)
{
	...
	l += snprint(p+l, READSTR-l, "Ru: %ud\n", ctlr->ru);
	l += snprint(p+l, READSTR-l, "Rps: %ud\n", ctlr->rps);
	l += snprint(p+l, READSTR-l, "Rwt: %ud\n", ctlr->rwt);
	l += snprint(p+l, READSTR-l, "Tps: %ud\n", ctlr->tps);
	l += snprint(p+l, READSTR-l, "Tu: %ud\n", ctlr->tu);
	l += snprint(p+l, READSTR-l, "Tjt: %ud\n", ctlr->tjt);
	l += snprint(p+l, READSTR-l, "Unf: %ud\n", ctlr->unf);
	...
	l += snprint(p+l, READSTR-l, "Descriptor Error: %ud\n", ctlr->de);
	l += snprint(p+l, READSTR-l, "Underflow Error: %ud\n", ctlr->uf);
	l += snprint(p+l, READSTR-l, "Excessive Collisions: %ud\n", ctlr->ec);
	l += snprint(p+l, READSTR-l, "Late Collision: %ud\n", ctlr->lc);
	l += snprint(p+l, READSTR-l, "No Carrier: %ud\n", ctlr->nc);
	l += snprint(p+l, READSTR-l, "Loss of Carrier: %ud\n", ctlr->lo);
	l += snprint(p+l, READSTR-l, "Transmit Jabber Timeout: %ud\n",
		ctlr->to);
	l += snprint(p+l, READSTR-l, "csr6: %ux %ux\n", csr32r(ctlr, 6),
		ctlr->csr6);
...
ctlrinit(Ether* ether)
{
	 ...
	 */
	ctlr->rdr = xspanalloc(ctlr->nrdr*sizeof(Des), 8*sizeof(u32int), 0);
	...
	ctlr->tdr = xspanalloc(ctlr->ntdr*sizeof(Des), 8*sizeof(u32int), 0);
