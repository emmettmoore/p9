...
mpstartap(Apic* apic)
{
	 ...
	 * The four pages are: pdb, pte0 (for MACHADDR's pte), Mach, and gdt.
	 */
	...
	*pte = PADDR(mach)|PTEWRITE|PTEVALID;
	...
	mach->pdpt = (PTE*)(p+(CPU0PDPT-CPU0GDT));	/* in page containing per mach gdt */
	pdptinit(mach->pdpt, pdb);
...
mpgo(Apic *bsp, int machno)
{
	...
	print("apic: %d machs started; %s mode vectors\n", conf.nmach, apicmode());
	/*
