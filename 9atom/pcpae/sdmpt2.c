 ...
 */
static int
port2unit(Ctlr *ctlr, uint devh, uint port, uint exph, uint ench)
{
	int unitidx, encidx;
	u32int slot, page[14];

	/* hack: ignore direct ports if controler has 1 port */
	if(ctlr->numports == 1)
		encidx = ench - 2;
	else
		encidx = ench - 1;
	...
	if(exph == 0 && ench == 1)
		...
		slot = page[2] & 0xffff;
		if(slot >= Nunitperenc)
			return -1;		/* too many drives per enclosure */
...
topoevent(Ctlr *ctlr, u32int *data)
{
	...
	for(i = 0; i < e; i++){
		...
		unitno = port2unit(ctlr, devh, i, exph, ench);
		if(unitno == -1 || unitno >= ctlr->sdev->nunit){
			print("   entry %d exph %d ench %d\n", i, exph, ench);
			continue;
		}
		u = UNIT(ctlr, unitno);
		...
		case 1:			/* online */
			...
			u->exph = exph;
			u->ench = ench;
			print("unit %d slot %d:%d devh %.4ux  rate %x stat %x online wwn %llux\n",
				i+b, ench, u->slot, devh, rate, *p>>28, u->wwn);
			u->state = Online;
			break;
		case 2:			/* offline */
...
encevent(Ctlr *, u32int *data)
{
	...
	print("mpt2: enc %d %s: port %d wwn %llux slots %d-%d phy %b\n", data[0]>>0 & 0xffff, reason,
		data[0]>>24, (uvlong)data[2]<<32 | data[1], s, s+n, data[4]);
}
...
doevent(Ctlr *ctlr, u32int *reply)
{
	...
	case SasEnclDeviceStatusChange:
		...
		break;
	default:
		panic("sdmpt2: doevent: %s: bad event %#.4ux", ctlr->name, event);
		break;
	}
