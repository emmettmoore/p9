...
mkvqueue(int size)
{
	...
	p = (uchar*)ROUNDUP(PTR2UINT(p), PGSZ);
	q->used = (void*)p;
	p += VringSize;
	q->usedent = (void*)p;
	p += sizeof(Vused)*size;
...
vioscsireq(SDreq *r)
{
	uchar resp[4+4+2+2+SENSESIZE], req[8+8+3+CDBSIZE];
	struct Rock rock;
	int free, head;
	u32int len;
	Vqueue *q;
...
viopnp(void)
{
	...
	h = nil;
	hh = &h;
	for(vd =  viopnpdevs(TypBlk); vd != nil; vd = vd->next){
		if(vd->nqueue != 1)
			continue;
