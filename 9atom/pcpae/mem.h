...
#define	BY2XPG		(2048*1024)
#define	BY2PTE		BY2V
#define	XPGSHIFT	21				/* log2(BY2XPG) */
#define	NPDPTE		4
#define	PDPTSIZE		(NPDPTE*BY2PTE)
#define	PTE2PG		(BY2PG/BY2PTE)
...
#define	NVPT		(1024*1024)	/* same for PAE (4*512*512) */
#define	VPTSIZE		(NVPT*BY2PTE)
#define	NVPTPDE		(VPTSIZE/BY2XPG)
#define	KMAP		(VPT-KMAPSIZE)
#define	KMAPSIZE	BY2XPG
#define	KXMAP		(VPT-KMAPSIZE-KXMAPSIZE)
#define	NKXMAP		4	/* how many big KMaps might be active at once */
#define	KXMAPSIZE	(NKXMAP*BY2XPG)
 ...
 */
#define	BOOTMEMSIZE	(8*1024*1024)
...
#define	CPU0PDB		(KZERO+0x12000)		/* bootstrap processor PDB (index 3 iwth PAE) */
#define	CPU0PTE		(CPU0PDB+BY2PG)		/* bootstrap processor PTEs  */
#define	CPU0GDT		(CPU0PTE+(((BOOTMEMSIZE/BY2PG)*BY2PTE)/BY2PG)*BY2PG)		/* bootstrap processor GDT */
#define	CPU0PDPT	(CPU0GDT+0x400)		/* bootstrap processor PDPT */
#define	MACHADDR	(CPU0GDT+BY2PG)		/* as seen by current processor */
#define	CPU0MACH	(MACHADDR+BY2PG)	/* Mach for bootstrap processor */
...
#define	CPUPDPT(n)	(CPU0PDPT+(n)*PDPTSIZE)
 ...
 * and page tables.
 */
#define	PDPTX(va)		((((ulong)(va))>>30) & 3)
#define	PDX(va)		((((ulong)(va))>>21) & 0x01FF)
#define	PTX(va)		((((ulong)(va))>>12) & 0x01FF)

#define	USEG	PDPTX(UZERO)
#define	KSEG	PDPTX(KZERO)	/* also includes space below KZERO; limits kern to 1gb */
