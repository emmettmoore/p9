...
static void*
_dp8390read(Dp8390* ctlr, void* to, uint from, uint len)
{
	uchar cr;
	int timo;
...
void*
dp8390read(Dp8390* ctlr, void* to, uint from, uint len)
{
	void *v;
...
static void*
dp8390write(Dp8390* ctlr, uint to, void* from, uint len)
{
	uint crda;
	uchar cr;
	int timo, width;
	...
	if(ctlr->dummyrr && (ctlr->width == 1 || ctlr->width == 2)){
		...
		for(timo=0;; timo++){
			...
			if(crda == to){
				...
				crda |= regr(ctlr, Crda1)<<8;
				if(crda != to)
					panic("crda write %ud to %ud", crda, to);
...
receive(Ether* ether)
{
	...
	for(curr = getcurr(ctlr); ctlr->nxtpkt != curr; curr = getcurr(ctlr)){
		...
		if(ctlr->ram)
			memmove(&hdr, UINT2PTR(ether->mem+data), sizeof(Hdr));
		else
		...
		if((hdr.status & (Fo|Fae|Crce|Prxok)) == Prxok && (bp = iallocb(len))){
			...
			if((data+len) >= ctlr->pstop*Dp8390BufSz){
				...
				if(ctlr->ram)
					memmove(p, UINT2PTR(ether->mem+data), count);
				else
			...
			if(len){
				if(ctlr->ram)
					memmove(p, UINT2PTR(ether->mem+data), len);
				else
