...
atageniostart(Drive* drive, uvlong lba)
{
	...
	case Cwsm:
		microdelay(1);
		/*
		 * std unclear: use similar timings to freebsd & linux:
		 * 1s to clear busy, 50ms to clear Drq.
		 */
		as = ataready(cmdport, ctlport, 0, Bsy, 0, 1*1050*1000);
		if(as < 0 || (as & Err)){
			iunlock(ctlr);
			return -1;
		}
