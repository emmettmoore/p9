...
enum {
	...
	Nrxblks		= 1024,
	Rbsize		= 9022,
	Rxblklen		= 2+Rbsize, 	/* ifc. supplies first 2 bytes as padding */
	...
	Descralign	= 16,
	Bufalign		= 8,
	Bufpercl		= CACHELINESZ/Descralign,
...
struct Ctlr {
	...
	uint	rxhead;		/* descr ethernet will write to next */
	uint	rxtail;		/* next descr that might need a buffer */
...
static void
replenish1(Ctlr *c, Block *b, uint t)
{
	Rx *r;
...
rxreplenish(Ctlr *c)
{
	...
	for(t = c->rxtail; (t - c->rxhead)%Nrx < Nrx - 8*Bufpercl; t = (t+i)%Nrx){
		for(i = 0; i < Bufpercl; i++){
			b[i] = rxallocb();
			if(b[i] == nil)
				break;
		}
		if(i != Bufpercl){
			iprint("#l%d: ether1116: rxreplenish out of buffers\n",
				c->ether->ctlrno);
			for(j = 0; j < i; j++)
				freeb(b[j]);
			break;
		...
		for(i = 0; i < Bufpercl; i++)
			replenish1(c, b[i], (t+i)%Nrx);
		cachedwbse(c->rx + t, CACHELINESZ);
		l2cacheuwbse(c->rx + t, CACHELINESZ);
	}
	c->rxtail = t;
}
...
ethercheck(Ether *e)
{
	if (e->link && e->starttime != 0 &&
	    5 + Ticks - e->starttime > Etherstuck*HZ) {
		e->starttime = 0;
...
static void
macerr(Ctlr *c, uint cs)
{
	switch(cs & RCSmacmask){
	case 0:
		c->rxcrcerr++;
		break;
	case RCSmacor:
...
//		assert(((uintptr)r & (Descralign - 1)) == 0);
		...
		if((cs = r->cs) & RCSdmaown)
			break;

		b = ctlr->rxb[ctlr->rxhead];
		if (b == nil)
			panic("ether1116: nil ctlr->rxb[ctlr->rxhead] in receive");
		ctlr->rxb[ctlr->rxhead] = nil;
		ctlr->rxhead = NEXT(ctlr->rxhead, Nrx);
		etheractive(ether);

		if((cs & (RCSfirst|RCSlast|RCSmacerr)) != (RCSfirst|RCSlast)){
			if(cs & RCSmacerr)
				macerr(ctlr, cs);
			else
				ctlr->rxnofirstlast++;
...
txreplenish(Ether *ether)			/* free transmitted packets */
{
	...
	while(ctlr->txtail != ctlr->txhead) {
		...
		if(ctlr->txb[ctlr->txtail] == nil)
			panic("no block for sent packet?! %d", ctlr->txtail);
...
//		assert(b->ref == 1);
	...
	ctlr->rx = xspanalloc(Nrx * sizeof(Rx), CACHELINESZ, 0);
	if(ctlr->rx == nil)
	...
	ctlr->tx = xspanalloc(Ntx * sizeof(Tx), CACHELINESZ, 0);
	if(ctlr->tx == nil)
...
//	archetheraddr(ether, ctlr->reg, Qno);	/* 2nd location */
	...
	reg->portcfg = Rxqdefault(Qno) | Rxqarp(Qno) | PCFGrxcs;
	reg->portcfgx = 0;
	coherence();
	...
	reg->psc0 = PSC0porton | 
		PSC0an_pauseadv | PSC0nofrclinkdown | PSC0mru(PSC0mru9022);
	coherence();
...
ifstat(Ether *ether, void *a, long n, ulong off)
{
	...
	p = seprint(p, e, "rx no first+last flag: %lud\n", ctlr->rxnofirstlast);
	p = seprint(p, e, "rx crcerr: %lud\n", ctlr->rxcrcerr);
	p = seprint(p, e, "rx overrun frames: %lud\n", ctlr->rxoverrun);
	p = seprint(p, e, "rx toolong: %lud\n", ctlr->rxtoolong);
	p = seprint(p, e, "rx bufoverrun: %lud\n", ctlr->rxbufoverrun);
	p = seprint(p, e, "rx nobufs: %lud\n", ctlr->rxnobufs);
