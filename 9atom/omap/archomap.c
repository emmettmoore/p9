...
struct Usbotg {
	...
	u32int	otgsyscfg;
	u32int	otgsyssts;
	u32int	otgifcsel;	/* interface selection */
	uchar	_pad1[0x414 - 0x410];
	u32int	otgforcestdby;
};
...
struct L3protreg {		/* hw: an L3 protection region */
	u64int	req_info_perm;
	u64int	read_perm;
	u64int	write_perm;
	u64int	addr_match;	/* ro? write this one last, then flush */
};
...
struct Cm {				/* clock management */
	u32int	fclken;			/* ``functional'' clock enable */
	u32int	fclken2;
	u32int	fclken3;
	uchar	_pad0[0x10 - 0xc];

	u32int	iclken;			/* ``interface'' clock enable */
	u32int	iclken2;
	u32int	iclken3;
	uchar	_pad1[0x20 - 0x1c];

	u32int	idlest;			/* idle status */
	u32int	idlest2;
	u32int	idlest3;
...
struct Gpio {
	...
	u32int	irqsts1;		/* for mpu */
	u32int	irqen1;
	u32int	wkupen;
	u32int	_pad1;
	u32int	irqsts2;		/* for iva */
	u32int	irqen2;
	...
	u32int	deben;			/* debouncing enable */
	u32int	debtime;
	u32int	_pad2[2];
