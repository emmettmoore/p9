...
getconf(char *name)
{
	...
	//	return "0 b115200 l8 pn m0 s1 i128 w100";	/* baud setting fails; don't do it */
		return "0 l8 pn m0 s1 i128 w100";
	return nil;	/* stub */
}
...
main(void)
{
	...
	fmtinit();
	vecinit();
	i8250console();			/* required for rbuart.c */
	normalprint = 1;
	print("\nPlan 9\n");
	prcpuid();
...
init0(void)
{
	...
	if(!waserror()){
		 ...
		 */
		ksetenv("nobootprompt", bootmethod, 0);	/* see rb config file */
...
userinit(void)
{
	...
	p->seg[SSEG] = s;
	pg = newpage(1, 0, USTKTOP-BY2PG, s->lgpgsize);
	segpage(s, pg);
	k = kmap(pg);
	bootargs(VA(k));
	kunmap(k);
...
confinit(void)
{
	 ...
	 */
	conf.mem[0].base = ktop = PADDR(ROUNDUP((uintptr)end, BY2PG));
