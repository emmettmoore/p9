...
athrbfree(Block* bp)
{
	bp->wp = bp->rp = bp->lim - ROUNDUP(Rbsz, BLOCKALIGN);
...
rxnewbuf(Ctlr *ctlr, int i)
{
	...
	if (ctlr->rd[i] != nil)
		panic("rxnewbuf: overflow"); //return;
...
etherintr(void *arg)
{
	...
	ilock(ctlr);
	sts = arge->dmaintrsts;
	if (sts & (Dmarxpktrcvd | Dmarxovflo)) {
		arge->dmaintr &= ~Dmarxpktrcvd;
		ctlr->pktstoread = 1;
		wakeup(&ctlr->rrendez);
		ctlr->rintr++;
		sts &= ~(Dmarxpktrcvd | Dmarxovflo);
	}
...
etherprobe(int ctlrno)
{
	...
	i += sprint(buf+i, " port %#P irq %d", PADDR(ether->port), ether->irq);
